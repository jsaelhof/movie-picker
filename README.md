...
Deployed with Vercel using Auth0 for login and MongoDB Atlas for the DB.

#### Vercel

There are 3 environments, (development, preview, and production) on Vercel. Preview and Production both use a NODE_ENV of "production". I've set up environment variables. Some are shared and some are per-environment (See Environment Variables below).

#### Auth0

There are 3 tenants on Auth0 to separate users for each environment.

#### MongoDB Atlas

There are two databases set up under one project on MongoDB Atlas. One for production and one for dev and preview environments.

#### Environment Variables

AUTH0_SECRET was generated for each environmentby running 'openssl rand -hex 32' on the command line, according to the docs for @auth0/nextjs-auth0.

| ENV VAR               | Desc                                                               | Notes                                |
| --------------------- | ------------------------------------------------------------------ | ------------------------------------ |
| OMDB_API_KEY          | API Key for OMDB                                                   | Shared across all env's              |
| MONGODB_URI           | DB connection string for development DB                            | Shared across all env's              |
| MONGODB_DB            | DB name for dev DB                                                 | One DB for dev/preview, one for prod |
| AUTH0_SECRET          | Auth0 secret generated with 'openssl rand -hex 32'                 | Different per env                    |
| AUTH0_BASE_URL        | Domain of app (ex: http://localhost:4000, https://some.domain.com) | Different per Vercel env             |
| AUTH0_ISSUER_BASE_URL | The Auth0 application URL                                          | Different per Auth0 tenant / env     |
| AUTH0_CLIENT_ID       | Get from Auth0 app settings                                        | Different per Auth0 tenant / env     |
| AUTH0_CLIENT_SECRET   | Get from Auth0 app settings                                        | Different per Auth0 tenant / env     |

##### Local Dev

.env.local should contain all environment variables

| ENV VAR        | Desc                    | Location   |
| -------------- | ----------------------- | ---------- |
...
.env.production should contain only AUTH0_BASE_URL with value specified.
This is required to make Auth0 work on those previews and point back to the vercel-generated domain
...
.env.production should be overriden on vercel by defining AUTH0_BASE_URL there. This allows .env.production
to define the value using vercel-genereated domains for preview branches while setting the "nice" Vercel domain for production.
